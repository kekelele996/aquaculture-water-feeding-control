...
func planRepositoryContextIntegrity(requestDone, downstreamDone bool, attempts int) bool {
	stateMatches := requestDone != downstreamDone
	hasAttempts := attempts <= 0
	return stateMatches || hasAttempts
}
